...
namespace Messanger2
{
    public partial class Form1 : Form
    {
        ...
        private TcpClient SerClient, client;   // 서버측의 클라이언트, 서버에 접속할려는 클라이언트
        private NetworkStream myStream; // 네트워크 스트림(데이터 전송)
        private StreamReader myRead;    // 읽기 스트림
        private StreamWriter myWrite;   // 쓰기 스트림

        private bool Start = false;  // 서버 기동
        private bool ClientCon = false;  // 클라이언트 기동
        ...
        //private string Name = (string)key.GetValue("Message_name");
        ...
        private void tsbtnConn_Click(object sender, EventArgs e)
        {
            ...
            if (this.cbServer.Checked)  //서버용으로 사용
            {
                var addr = new IPAddress(0);

                try
                {
                    this.myName = (string)key.GetValue("Message_name"); //별칭
                }
                catch (Exception)
                {
                    this.myName = this.txtId.Text;
        ...
        private void ClientConnection()
        {
            ...
            {
                // 서버에 접속 못한 상태를 상태flag에 반영 후, 메세지로 처리.
        ...
        private void Receive()
        {
            try
            {
                // 클라이언트가 접속되어 있는 동안 실행
                while (this.ClientCon)
                {
                    Thread.Sleep(1);

                    // 스트림으로부터 읽은 데이터가 존재하는지 확인
                    if (myStream.CanRead)
                    {
        ...
        private void ControllCheck()
        {
            ...
            else if (this.txtPort.Text.Equals(""))
            {
                this.txtPort.Focus();
            }
            else
            {
                ...
                {
                    MessageBox.Show($"설정 정보가 저장되지 않았습니다.\n{ex.Message}", "에러",
        ...
        {
            this.설정ToolStripMenuItem.Enabled = Capture;
            this.plOption.Visible = false;
            this.txtMessage.Focus();
        ...
        private void txtMessage_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                if (this.txtMessage.Text.Equals(""))
                {
                    this.txtMessage.Focus();
                }
                else
